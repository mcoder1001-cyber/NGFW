...
// mactimeIsEnabled answers feature_is_enabled for the mactime feature through the TD-23 composition seam
// (RegisterFeatureIsEnabled), so it composes with other features' answers instead of replacing the handler.
func mactimeIsEnabled(v *VPP, r *feature.FeatureIsEnabled) *feature.FeatureIsEnabledReply {
	v.mu.Lock()
	defer v.mu.Unlock()
	m := v.l2()
	v.gcLocked(m)
	...
	return &feature.FeatureIsEnabledReply{IsEnabled: m.mtCount[idx] > 0 || !m.reached[idx]}
}

func init() {
	RegisterExtension("bridge-l2", (*VPP).installBridgeL2)
	RegisterFeatureIsEnabled("mactime", mactimeIsEnabled)
}
