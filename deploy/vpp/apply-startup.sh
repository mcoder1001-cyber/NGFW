...
# --i-have-product-owner-approval PENDING-<slug>, which must name docs/decisions/PENDING-<slug>.md on
# main of /root/ngfw AND a LOG.md D-row on main that references it (both verified, recorded, logged).
#
...
#       manager and exact restore plan; the management REACHABILITY CHECK that will be used (and exit 3
#       when none is viable on this host); a read-only VPP preflight; the gate; the sha256 of the live
#       file and of the rendering. Changes nothing. Exit 3 when --apply would refuse.
#   apply-startup.sh --doc <document.json> --apply --expect-sha256 <live sum> --expect-new-sha256 <rendered sum>
#       APPLY: the planner (this process) evaluates the gate, copies the document, the generator,
#       vrx-vppcheck and this script into a work dir, records every setting there, seals the plan
#       (<work>/plan.sha256 = sha256 over settings, document, generator args, binaries and gate record)
#       and re-runs itself DETACHED from the SSH session (systemd-run --unit=vrx-startup-apply-<stamp>,
#       settings also passed as --setenv; fallback setsid). The detached run:
#         1. starts a LOCK HOLDER (its own unit/session) that takes flock -x on /run/lock/vrx-vpp.lock then
#            /run/lock/vrx-lab.lock (tools/lab order, bounded wait) and keeps them until the apply has
#            committed or rolled back — the locks never go free in between, not even when the run dies
#         2. verifies the plan seal and re-evaluates the gate (must equal the sealed record); refuses
#            unless sha256(live) == --expect-sha256 and sha256(rendering) == --expect-new-sha256; VPP
#            preflight; the reachability check must pass now (baseline)
#         3. snapshots: per management interface `ip -j addr` / `ip -j route` + exact restore plan, NIC
#            drivers, loaded plugins, VPP boot identity (D-080: boot_id/PID/start time); backup; diffs
#         4. arms a dead-man timer (systemd-run --on-active; fallback setsid)
#         5. installs, restarts VPP, then requires: a NEW VPP boot identity whose PID is vpp.service's
#            MainPID, then for --window seconds: unit active, MainPID + ActiveEnterTimestampMonotonic and
#            the boot identity unchanged (no crash/restart since — NRestarts is NOT used: systemd resets it
#            on a manual restart), plugins per `show plugins`, logical interfaces per sw_interface_dump,
#            every management interface up with addresses and routes exactly as snapshotted, NIC driver
#            unchanged, and the reachability check passing
#         6. any failure → rollback: stop VPP (SIGKILL if stop hangs), restore the backup, rebind changed
#            NICs, re-apply the recorded addresses/routes verbatim, then only if still broken the network
#            manager (ifupdown `ifup --force` / networkd `networkctl reconfigure` / `netplan apply`),
#            reset-failed + start VPP, verify the same way. Healthy → locks released, timer cancelled.
#            Incomplete → locks stay held; the dead-man retries.
#       Every VPP/kernel/systemd call has a timeout (--cmd-timeout, --svc-timeout). The run's budget plus a
#       worst-case rollback fit before the dead-man fires. The dead-man KILLS the run (its process tree,
#       never the lock holder), rolls back with up to --rollback-retries attempts (exponential backoff from
#       --retry-backoff), then releases the locks. If the holder is gone it first takes the locks itself,
#       exclusively and bounded, before killing the run; only a foreign holder makes it roll back without
#       them (FORCED, holder logged).
#   apply-startup.sh --stage run|rollback|hold --work <dir>     internal (the detached parts)
#
# Options: --vm NAME (vrx-a) · --window S (60) · --interval S (5) · --api-wait S (60) · --cmd-timeout S (10) ·
#   --svc-timeout S (120) · --lock-timeout S (600) · --deadman-lock-timeout S (60) · --rollback-retries N (3) ·
#   --retry-backoff S (10) · --mgmt-if IF (repeatable) · --mgmt-peer IP (the SSH client of $SSH_CONNECTION is
#   added automatically) · --mgmt-probe auto|ssh-peer|gateway-ping|tcp:HOST:PORT (auto: ssh-peer when the
#   peer's session is ESTABLISHED, else gateway-ping when the gateway answers ICMP now, else refuse) ·
#   --foreground (run the detached stage in this process; refused over SSH unless --console) ·
#   --i-have-product-owner-approval PENDING-<slug>
#
# Logs and state: /var/lib/vrx/startup-apply/<stamp>/ (log, settings, gate, plan.sha256, doc.json, bin/, new.conf,
# backup.conf, drivers, mgmt.ifs, mgmt/<if>.*, ident.*, plugins.before, installed, committed | rolled-back |
# rollback-incomplete). Syslog tag vrx-startup-apply.
# Exit: 0 ok / nothing to do · 1 failed and rolled back · 2 usage · 3 refused before any change.
#
# Every system path and command is overridable through VRX_* variables (below) so the script is tested
# against a fake host (deploy/vpp/test-apply-startup.sh). Workers never run --apply for real.
set -euo pipefail
...
SETTINGS_VARS=(VRX_STARTUP_CONF VRX_SYSFS VRX_ETC VRX_APPLY_STATE VRX_LAB_LOCK VRX_VPP_LOCK VRX_VPP_API_SOCKET
  ...
  VRX_LOGGER VRX_LSLOCKS VRX_VPPCHECK VRX_STARTUPGEN VRX_HANDOVER_EXTRA VRX_MGMT_PEERS VRX_MGMT_IFS VRX_MGMT_PROBE
  WINDOW INTERVAL API_WAIT CMD_TIMEOUT SVC_TIMEOUT LOCK_TIMEOUT DEADMAN_LOCK_TIMEOUT RB_RETRIES RB_BACKOFF
  EXPECT EXPECT_NEW VM APPROVAL GATE_REPO)
CANON_ROOT=/root/ngfw   # the manager's main checkout: its handover flag and decisions always count; no override
...
CMD_TIMEOUT=10 SVC_TIMEOUT=120 RB_RETRIES=3 RB_BACKOFF=10 FOREGROUND=0 CONSOLE=0 STAGE="" WORK="" DELAY=0 VM="vrx-a"
APPROVAL="" GATE="" GATE_REPO=""
GEN_ARGS=()
...
parse_args() {
  while (($#)); do
    case "$1" in
      ...
      --rollback-retries) RB_RETRIES="${2:?}"; shift ;;
      --retry-backoff) RB_BACKOFF="${2:?}"; shift ;;
      --vm) VM="${2:?}"; shift ;;
  ...
  local n w
  for n in WINDOW INTERVAL LOCK_TIMEOUT DEADMAN_LOCK_TIMEOUT API_WAIT CMD_TIMEOUT SVC_TIMEOUT RB_RETRIES RB_BACKOFF DELAY; do
    [[ ${!n} =~ ^[0-9]+$ ]] || die "--${n,,} must be a number"
  done
  ((CMD_TIMEOUT > 0 && SVC_TIMEOUT > 0 && RB_RETRIES > 0)) || die "timeouts and --rollback-retries must be > 0"
  ...
  [[ $VRX_MGMT_PROBE =~ ^(auto|ssh-peer|gateway-ping|tcp:(\[[0-9A-Fa-f:.]+\]|[0-9A-Za-z.-]+):[0-9]{1,5})$ ]] ||
    die "--mgmt-probe must be auto, ssh-peer, gateway-ping or tcp:HOST:PORT"
...
unit_ident() {  # vpp.service main process + when it became active (changes on every start, crash or restart)
  tmo "$VRX_SYSTEMCTL" show vpp -p MainPID -p ActiveEnterTimestampMonotonic 2>/dev/null | sort | tr '\n' ' '
}
...
netmgr_of() {  # <if> → ifupdown | netplan | networkd | none (who re-applies the interface's configuration)
  local dev f re; re="$(ere_escape "$1")"; dev="$1"
...
# ---------------------------------------------------------------- management reachability (review H2)
...
live_gateways() {  # → "<dev> <gw>" of every default route (v4+v6)
  { tmo "$VRX_IP" -j -4 route show default 2>/dev/null || echo '[]'; tmo "$VRX_IP" -j -6 route show default 2>/dev/null || echo '[]'; } |
    jq -r '.[]? | select(.gateway != null and .dev != null) | "\(.dev) \(.gateway)"' 2>/dev/null || true
}
probe_ok() {  # <probe> → 0 when the management path answers (prints why not)
  local p="$1" peer dev gw hp host port n=0
  case "$p" in
    ssh-peer)
      for peer in $VRX_MGMT_PEERS; do
        if [[ -n $(tmo "$VRX_SS" -Htn state established dst "$peer" 2>/dev/null) ]]; then return 0; fi
      done
      echo "no ESTABLISHED TCP session with the manager (${VRX_MGMT_PEERS:-no peer})"; return 1 ;;
    gateway-ping)
...
resolve_probe() {  # VRX_MGMT_PROBE → a probe that passes NOW (the baseline), or fail (prints why)
  ...
  if [[ -n $VRX_MGMT_PEERS ]] && probe_ok ssh-peer >/dev/null; then echo ssh-peer; return 0; fi
  if probe_ok gateway-ping >/dev/null; then echo gateway-ping; return 0; fi
  echo "no viable management reachability check on this host: no ESTABLISHED session with a known manager peer (${VRX_MGMT_PEERS:-none}) and the gateway does not answer ICMP — pass --mgmt-probe tcp:HOST:PORT (a TCP service reached through the management interface)" >&2
  return 1
}
...
# ---------------------------------------------------------------- handover gate (D-012; re-review N8, review M2)
...
handover_sources() {  # → "<source> <pending|done>" lines: the canonical copy always, the planner's tree, extras
  local rel="docs/lab/host-$VM.md" f
  if [[ -r $CANON_ROOT/$rel ]]; then echo "$CANON_ROOT/$rel $(handover_flag < "$CANON_ROOT/$rel")"
  else echo "$CANON_ROOT/$rel(absent) pending"; fi
  if [[ -n $GATE_REPO && $GATE_REPO != "$CANON_ROOT" && -r $GATE_REPO/$rel ]]; then echo "$GATE_REPO/$rel $(handover_flag < "$GATE_REPO/$rel")"; fi
  for f in $VRX_HANDOVER_EXTRA; do
    if [[ -r $f ]]; then echo "$f $(handover_flag < "$f")"; else echo "$f(absent) pending"; fi
  done
}
approval_ref() {  # APPROVAL → "docs/decisions/PENDING-x.md @<blob> + LOG D-nnn" from main of the canonical repo, or fail
  local file="docs/decisions/$APPROVAL.md" blob ds re
  blob="$(git -c safe.directory="$CANON_ROOT" -C "$CANON_ROOT" rev-parse --verify -q "main:$file" 2>/dev/null)" ||
    { echo "$file does not exist on main of $CANON_ROOT"; return 1; }
  re="$(ere_escape "$APPROVAL")"
  ds="$(git -c safe.directory="$CANON_ROOT" -C "$CANON_ROOT" show main:docs/decisions/LOG.md 2>/dev/null | grep -E "(^|[^A-Za-z0-9_-])$re([^A-Za-z0-9_-]|$)" |
    grep -oE '^\|[^|]*\| *D-[0-9]+' | grep -oE 'D-[0-9]+' | sort -u | tr '\n' ' ' || true)"
  [[ -n $ds ]] || { echo "no D-row in docs/decisions/LOG.md on main references $APPROVAL"; return 1; }
  echo "$file@${blob:0:12} + LOG ${ds% }"
}
gate() {  # → the gate record (deterministic: the stage recomputes and compares it); returns 3 when --apply is not allowed
...
dry_run() {
  ...
  if probe="$(resolve_probe 2>"$tmp/probe.err")"; then echo "  will use: $probe (passes now)"
...
# ---------------------------------------------------------------- locks (review M1: a separate holder keeps them)
...
start_holder() {  # → 0 when the holder owns both locks, 3 when they stayed busy
  ...
  else setsid "$WORK/bin/apply-startup.sh" --stage hold --work "$WORK" </dev/null >/dev/null 2>&1 8>&- 9>&- & fi
...
stage_hold() {
  ...
  while ((SECONDS < end)) && [[ ! -e $WORK/release ]]; do sleep 1 8>&- 9>&-; done
...
check_mgmt() {  # every protected interface as snapshotted + same driver; then the reachability check
  local dev pci why
  while read -r dev; do
...
vpp_restarted_ok() {  # right after the restart: a NEW VPP instance, and it is vpp.service's main process (review H1)
  local before now pid unit
  before="$(cat "$WORK/ident.before")"
  now="$(vppcheck bootid 2>/dev/null)" || { echo "VPP boot identity unreadable"; return 1; }
...
check_health() {  # → prints the first problem, returns 1
  ...
  [[ $(unit_ident) == "$(cat "$WORK/unit.after")" ]] || { echo "vpp.service restarted since the apply ($(cat "$WORK/unit.after")→ $(unit_ident))"; return 1; }
  [[ -S $VRX_VPP_API_SOCKET ]] || { echo "no API socket $VRX_VPP_API_SOCKET"; return 1; }
  local id loaded p names out
  id="$(vppcheck bootid 2>/dev/null)" || { echo "VPP API does not answer within ${CMD_TIMEOUT}s (hung?)"; return 1; }
...
cancel_deadman() {
  ...
  if [[ -n $p && $p != "$$" ]] && proc_is "$p" "--stage rollback --work $WORK"; then kill "$p" 2>/dev/null || true; fi
}
rollback_once() {  # restore file + drivers + management path, restart VPP, verify → 0 healthy / 1 not
  say "ROLLBACK: $1"
  ...
  if [[ -z $why ]]; then
    rm -f "$WORK/rollback-incomplete"; touch "$WORK/rolled-back"
  ...
  say "ROLLBACK INCOMPLETE ($why)"
  syslog "ROLLBACK INCOMPLETE $WORK: $why"
  return 1
}
finish_rollback() {  # the rollback is over (healthy, or out of retries): stop the timer, free the locks
  cancel_deadman
  release_locks
}

# ---------------------------------------------------------------- budgets (review M3)
# worst-case durations with the configured timeouts; the dead-man fires only after run + one rollback
budgets() {
  ITER=$((6 * (CMD_TIMEOUT + 4) + 4 * CMD_TIMEOUT + INTERVAL))           # one check_health round
  RB_BUDGET=$((3 * SVC_TIMEOUT + 10 + API_WAIT + 30 * CMD_TIMEOUT + 30)) # stop + kill + netdev wait + start + checks
  RUN_BUDGET=$((SVC_TIMEOUT + 2 * (CMD_TIMEOUT + 4) + API_WAIT + WINDOW + ITER))
  DEADMAN_AFTER=$((RUN_BUDGET + ITER + RB_BUDGET + 60))
  HOLD_MAX=$((DEADMAN_AFTER + RB_RETRIES * (RB_BUDGET + RB_BACKOFF * (1 << RB_RETRIES)) + 600))
}
...
detach() {  # the planner: pin and seal everything into the work dir, then run "--stage run" outside the SSH session
...
stage_run() {
  ...
  # review M2: the plan must be exactly what the planner sealed, and the gate must still hold
  ...
  start_holder || { refuse "locks busy"; return 3; }
  ...
  vppcheck bootid > "$WORK/ident.before" || { refuse "VPP boot identity unreadable"; return 3; }
  ...
  say "recorded drivers: $(tr '\n' ';' < "$WORK/drivers") VPP identity: $(cat "$WORK/ident.before") reachability check: $probe"
  while read -r dev; do
  ...
  if ! tmo "$VRX_SYSTEMD_RUN" --unit="$deadman" --on-active="$DEADMAN_AFTER" --timer-property=AccuracySec=1s --collect --quiet "${envs[@]}" \
    ...
    : > "$WORK/deadman-unit"
    setsid "$WORK/bin/apply-startup.sh" --stage rollback --work "$WORK" --delay "$DEADMAN_AFTER" </dev/null >/dev/null 2>&1 8>&- 9>&- &
  ...
  elif ! wait_api; then why="VPP API did not come up within ${API_WAIT}s (hung or crashed)"
  elif ! why="$(vpp_restarted_ok)"; then :
  else
    why=""
    say "VPP restarted: identity $(cat "$WORK/ident.after"); unit $(cat "$WORK/unit.after")"
  ...
  if [[ -n $why ]]; then
    # one attempt here (it fits before the dead-man); if incomplete the locks stay held and the dead-man retries
    if rollback_once "$why"; then finish_rollback; else say "the dead-man retries at its deadline (locks stay held)"; fi
    return 1
  fi
...
kill_run() {  # the run is stuck or dead: kill it and everything it started — except the lock holder
  local pid hold c i
  read -r pid _ < "$WORK/run-pid" 2>/dev/null || return 0
  proc_is "$pid" "--stage run --work $WORK" || return 0
  hold="$(cat "$WORK/locks-held" 2>/dev/null || true)"
  say "dead-man: killing the run (pid $pid) and its children"
  kill -STOP "$pid" 2>/dev/null || true
  for c in $(descendants "$pid"); do [[ $c == "$hold" ]] || kill -KILL "$c" 2>/dev/null || true; done
...
stage_rollback() {  # the dead-man
  ...
  kill_run
  if done_already; then say "dead-man: the run finished while being stopped — nothing to do"; release_locks; return 0; fi
  ...
  local attempt backoff=$RB_BACKOFF
  for ((attempt = 1; attempt <= RB_RETRIES; attempt++)); do
    if rollback_once "dead-man (attempt $attempt/$RB_RETRIES): the apply did not commit"; then finish_rollback; return 1; fi
    ((attempt < RB_RETRIES)) || break
    say "dead-man: retry in ${backoff}s"
    sleep "$backoff"; backoff=$((backoff * 2))
  done
  say "dead-man: ROLLBACK INCOMPLETE after $RB_RETRIES attempts — console access needed; releasing the locks"
  syslog "dead-man gave up after $RB_RETRIES attempts $WORK — console needed"
  finish_rollback
  return 1
}
