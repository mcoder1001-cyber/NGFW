# DF-8 — Descriptors: dhcp, dns, flowprobe/ipfix, sflow, prom, pcap/trace, lcp
...
23 descriptors in 9 packages under `apps/agent/internal/descriptors/` + the shared helper package `dfkit`
...
flowprobe,sflow,prom,pcap,trace,lcp}.md`.
...
| prom | — | `prom.http-static-server` | write-only; prom exporter not API-configurable |
...
- **Missing in VPP 26.06 on this host** (Q1): tracedump (`trace_set_filters`, `trace_v2_dump`, …) and tracenode are
  not built — no binapi; Trace Path has no API; the prom exporter has no API (CLI/startup.conf only). No descriptors;
  documented in trace.md / prom.md.
