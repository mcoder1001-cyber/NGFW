...
export function vrfStaticEcmpFake(agent: FakeAgent): {
  ...
  registerActionHandler('ping', vrfStaticEcmpAction); // TD-23: the dispatcher routes Action by its oneof case
  registerActionHandler('traceroute', vrfStaticEcmpAction);
  return {
    listRoutes: (call, cb) => {
...
/**
 * The ping and traceroute Action handlers, registered with the fake agent's action dispatcher (TD-23: features call
 * registerActionHandler, never override `action`); the dispatcher records the call. Traceroute: UNIMPLEMENTED.
 */
const vrfStaticEcmpAction: ActionHandler = (call) => {
