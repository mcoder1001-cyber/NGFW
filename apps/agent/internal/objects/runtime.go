...
type Config struct {
	...
	// MaxStale is how long a failing name keeps its last good answers (0 = DefaultMaxStale, 24 h),
	// clamped to [MinMaxStale, MaxMaxStale] (D-129).
...
// and the FQDN resolver over it. subsystems/object_model.go opens it when the family registers and
// closes it through the Wiring close seam (Wiring.OnClose, called by Agent.Stop).
type Runtime struct {
	key   string
	store *Store
	res   *resolver
...
// and registers the runtime for RuntimeFor (the state RPC's way in while agent core does not hand the
// Wiring to the gRPC server). A runtime still open for the same state dir and owner (an agent restarted
// inside one process without Stop, e.g. unit tests) is closed first, so its pending writes land before
// the files are read. The resolver does not run until Start.
func Open(cfg Config) (*Runtime, error) {
	...
	key := runtimeKey(cfg.StateDir, cfg.Owner)
	if prev := RuntimeFor(cfg.StateDir, cfg.Owner); prev != nil {
		prev.Close()
	}
	store, err := OpenStore(filepath.Join(cfg.StateDir, "objects-"+cfg.Owner+".json"), cfg.Log)
	if err != nil {
		return nil, err
	}
	res := newResolver(filepath.Join(cfg.StateDir, "objects-fqdn-"+cfg.Owner+".json"), cfg.Lookup, cfg.Refresh, cfg.MaxStale, cfg.Now, cfg.Log)
	res.load()
	rt := &Runtime{key: key, store: store, res: res, log: cfg.Log}
	store.onFQDN = func(name, host string) {
		if host == "" {
			res.untrack(name)
		} else {
			res.track(name, host)
...
// Start runs the resolver loop in the background (idempotent; a closed runtime does not start).
func (rt *Runtime) Start() {
	rt.mu.Lock()
	defer rt.mu.Unlock()
	if rt.cancel != nil || rt.closed {
		return
	}
...
// Close stops the resolver loop, writes the pending store changes and FQDN state, and unregisters the
// runtime (idempotent). The store stays readable.
func (rt *Runtime) Close() {
	rt.mu.Lock()
	cancel, done := rt.cancel, rt.done
	rt.cancel, rt.done, rt.closed = nil, nil, true
	rt.mu.Unlock()
	if cancel != nil {
		cancel()
		<-done
	}
	_ = rt.store.Close() // logged and counted by the store
	rt.res.persistIfDirty()
	runtimesMu.Lock()
...
// ResolveDue resolves every FQDN whose refresh is due now and writes the state file (synchronously;
// tests with a fake clock). It returns the number of names resolved.
func (rt *Runtime) ResolveDue(ctx context.Context) int {
	n := rt.res.resolveDue(ctx, 0)
	rt.res.persistIfDirty()
	return n
}
