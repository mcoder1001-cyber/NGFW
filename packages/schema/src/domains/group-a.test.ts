...
describe('system', () => {
  ...
  it('rejects unknown keys and bad values with the offending path', () => {
    ...
    expect(failPath(SystemSchema, { ntp: { servers: [{ address: 'bad host!' }] } })).toEqual(['ntp', 'servers', '0', 'address']);
    expect(failPath(SystemSchema, { banner: { login: 'x'.repeat(4097) } })).toEqual(['banner', 'login']);
  });
});
...
describe('dataplane', () => {
  it('accepts the empty section and enforces bounds', () => {
    ...
    expect(DataplaneSchema.safeParse({ workers: 4, corelist: [2, 3, 4, 5], mainCore: 1, rxQueues: 2, txQueues: 2, hugepagesGb: 4 }).success).toBe(true);
...
describe('interfaces', () => {
  it('keys are parent names; sub-interface ids are decimal u32', () => {
    ...
    expect(failPath(InterfacesSchema, { 'TenGigabitEthernet0/0/0.100': {} })).toEqual(['TenGigabitEthernet0/0/0.100']);
    for (const ok of ['0', '100', '4294967295']) expect(subInterfaceId.safeParse(ok).success).toBe(true);
    for (const bad of ['', '01', '-1', '1.5', '4294967296', '10000000000', 'a']) expect(subInterfaceId.safeParse(bad).success).toBe(false);
  });

  it('applies defaults and rejects unknown keys', () => {
    expect(InterfaceSchema.parse({})).toEqual({ enabled: false, promiscuous: false, ipv4: [], ipv6: [], vrf: 'default', subinterfaces: {} });
    expect(SubinterfaceSchema.parse({ vlanId: 5 })).toEqual({ vlanId: 5, dot1ad: false, enabled: false, ipv4: [], ipv6: [], vrf: 'default' });
  ...
  it('forbids own addresses on unnumbered (sub-)interfaces', () => {
    ...
    expect(failPath(InterfaceSchema, { unnumbered: 'loop0', ipv4: ['10.0.0.1/24'] })).toEqual(['unnumbered']);
    expect(failPath(SubinterfaceSchema, { vlanId: 1, unnumbered: 'loop0', ipv6: ['2001:db8::1/64'] })).toEqual(['unnumbered']);
  });

  it('enumerates parent and sub-interface names', () => {
    const parsed = InterfacesSchema.parse({ 'TenGigabitEthernet0/0/0': { subinterfaces: { '100': { vlanId: 100 } } }, loop0: {} });
    expect([...interfaceNames(parsed)].sort()).toEqual(['TenGigabitEthernet0/0/0', 'TenGigabitEthernet0/0/0.100', 'loop0']);
...
describe('routing', () => {
  it('next hops need an address or an interface; families must match the prefix', () => {
    expect(NextHopSchema.parse({ address: '10.0.0.1' })).toEqual({ address: '10.0.0.1', weight: 1 });
    ...
    expect(StaticRouteSchema.safeParse({ prefix: '10.0.0.0/8', nextHops: [{ address: '10.1.1.1' }, { interface: 'loop0' }] }).success).toBe(true);
    expect(StaticRouteSchema.safeParse({ prefix: '::/0', nextHops: [{ address: 'fe80::1', interface: 'loop0' }] }).success).toBe(true);
    expect(failPath(StaticRouteSchema, { prefix: '10.0.0.0/8', nextHops: [{ address: '2001:db8::1' }] })).toEqual(['nextHops']);
    expect(failPath(StaticRouteSchema, { prefix: '10.0.0.1/8', nextHops: [{ address: '10.1.1.1' }] })).toEqual(['prefix']);
    expect(failPath(StaticRouteSchema, { prefix: '10.0.0.0/8', nextHops: [] })).toEqual(['nextHops']);
  });

  it('prefix lists: unique sequences, family-consistent prefixes, ge ≤ le', () => {
    expect(PrefixListSchema.parse({})).toEqual({ family: 'ipv4', rules: [] });
    expect(PrefixListRuleSchema.safeParse({ seq: 1, action: 'permit', prefix: '10.0.0.0/8', ge: 16, le: 24 }).success).toBe(true);
    expect(failPath(PrefixListRuleSchema, { seq: 1, action: 'permit', prefix: '10.0.0.0/8', ge: 24, le: 16 })).toEqual(['le']);
    expect(failPath(PrefixListRuleSchema, { seq: 0, action: 'permit', prefix: '10.0.0.0/8' })).toEqual(['seq']);
    expect(failPath(PrefixListSchema, { rules: [{ seq: 1, action: 'permit', prefix: '10.0.0.0/8' }, { seq: 1, action: 'deny', prefix: '0.0.0.0/0' }] })).toEqual(['rules']);
    expect(failPath(PrefixListSchema, { family: 'ipv6', rules: [{ seq: 1, action: 'permit', prefix: '10.0.0.0/8' }] })).toEqual(['rules']);
    expect(PrefixListSchema.safeParse({ family: 'ipv6', rules: [{ seq: 1, action: 'permit', prefix: '2001:db8::/32' }] }).success).toBe(true);
  });
  ...
  it('route maps: unique sequences, empty match/set allowed', () => {
    ...
    expect(failPath(RouteMapSchema, { entries: [{ seq: 10, action: 'permit' }, { seq: 10, action: 'deny' }] })).toEqual(['entries']);
    expect(failPath(RouteMapSchema, { entries: [{ seq: 10, action: 'permit', set: { community: ['65000:70000'] } }] })).toEqual(['entries', '0', 'set', 'community', '0']);
    for (const ok of ['65000:100', '0:0', 'no-export', 'internet']) expect(bgpCommunity.safeParse(ok).success).toBe(true);
    for (const bad of ['65000', '65000:', 'a:b', '65536:1x', 'NO-EXPORT']) expect(bgpCommunity.safeParse(bad).success).toBe(false);
  });

  it('bgp: hold time above keepalive, remoteAs or peer group', () => {
    expect(BgpNeighborSchema.safeParse({ address: '10.0.0.1', remoteAs: 65001, keepaliveSec: 30, holdTimeSec: 90 }).success).toBe(true);
    expect(failPath(BgpNeighborSchema, { address: '10.0.0.1', remoteAs: 65001, keepaliveSec: 90, holdTimeSec: 90 })).toEqual(['holdTimeSec']);
    ...
    expect(failPath(BgpPeerGroupSchema, { keepaliveSec: 30, holdTimeSec: 10 })).toEqual(['holdTimeSec']);
  ...
  it('ospf: area ids normalise and must be unique; isis NET format', () => {
    ...
    for (const ok of ['49.0001.1921.6800.1001.00', '49.0001.0000.0000.0001.00']) expect(isisNet.safeParse(ok).success).toBe(true);
    for (const bad of ['49.0001.1921.6800.1001', '49.1.2.3.00', '', 'gg.0001.1921.6800.1001.00']) expect(isisNet.safeParse(bad).success).toBe(false);
  });

  it('bfd: local and peer address in one family, VPP-scale intervals', () => {
    expect(BfdSessionSchema.parse({ interface: 'loop0', localAddress: '10.0.0.1', peerAddress: '10.0.0.2' })).toMatchObject({
      desiredMinTxUs: 300000,
      requiredMinRxUs: 300000,
      detectMultiplier: 3,
      enabled: true,
    });
    expect(failPath(BfdSessionSchema, { interface: 'loop0', localAddress: '10.0.0.1', peerAddress: '2001:db8::2' })).toEqual(['peerAddress']);
    expect(failPath(BfdSessionSchema, { interface: 'loop0', localAddress: '10.0.0.1', peerAddress: '10.0.0.2', desiredMinTxUs: 1 })).toEqual(['desiredMinTxUs']);
  });
});

describe('management', () => {
  it('users: scope is always *, hash write-only, keys validated', () => {
    expect(UserSchema.parse({ username: 'admin', role: 'admin' })).toEqual({ username: 'admin', role: 'admin', scope: '*', sshKeys: [], disabled: false });
    expect(failPath(UserSchema, { username: 'admin', role: 'admin', scope: 'tenant-a' })).toEqual(['scope']);
    ...
    expect(failPath(UserSchema, { username: 'admin', role: 'admin', passwordHash: 'plaintext' })).toEqual(['passwordHash']);
    expect(sshPublicKey.safeParse('ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIFakeKeyForFixturesOnly0000000000000000000000 a@b').success).toBe(true);
    expect(sshPublicKey.safeParse('ecdsa-sha2-nistp256 AAAA==').success).toBe(true);
    for (const bad of ['ssh-dss AAAA', 'ssh-ed25519', 'ssh-ed25519 AAAA\nssh-rsa BBBB', 'AAAA']) expect(sshPublicKey.safeParse(bad).success).toBe(false);
  });

  it('aaa: unique order, servers present for listed methods', () => {
    expect(AaaSchema.parse({})).toEqual({ order: ['local'], radius: { servers: [] }, tacacs: { servers: [] } });
  ...
  it('tls: certificate and key come together', () => {
    ...
    expect(TlsSchema.safeParse({ certificateRef: 'tls/api/cert', privateKeyRef: 'tls/api/key', minVersion: '1.3' }).success).toBe(true);
...
describe('RootConfig with group (a) domains', () => {
  it('rejects a secret written inline where a secretRef is expected and reports the pointer path', () => {
    const r = RootConfig.safeParse({ management: { aaa: { radius: { servers: [{ address: '10.0.0.1', secretRef: 'hunter2 plain' }] } } } });
    expect(r.error?.issues[0]?.path).toEqual(['management', 'aaa', 'radius', 'servers', 0, 'secretRef']);
  });
});
