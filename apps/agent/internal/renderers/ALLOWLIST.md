...
| `/usr/bin/ip` | kea — agent in **lab test mode** only (F-kea-dhcp-relay, review Q7): the agent binary builds a runner that adds `ip` to `kea.Binaries()` only when `VRX_KEA_MODE=test`; `subsystems.registerKea` refuses that mode for the product agent (owner `vrx` or `VRX_VPP_ID_RANGE=all`), so the product runner never has it (`TestRegisterKeaModes`) | run the DHCP checkers inside the slot's Kea netns (`VRX_KEA_NETNS`, validated `^[a-z0-9][a-z0-9-]{0,31}$`) | `ip netns exec <ns> /usr/sbin/kea-dhcp4\|kea-dhcp6 -t <staged file>` (fixed argv, no shell) | F-kea-dhcp-relay |
