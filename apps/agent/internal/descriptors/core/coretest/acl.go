...
const (
	RetvalACLInvalidValue    int32 = -7
	RetvalACLInvalidValue2   int32 = -8
...
// AddTagged inserts an ACL with only its owner tag (no rules) and returns its index. F-rpf-adl-pbr's
// coretest helper VPP.AddACL uses it so both features share one acl_dump model (TD-23: acl.go owns acl_dump).
func (m *ACLModel) AddTagged(tag string) uint32 {
	m.mu.Lock()
	defer m.mu.Unlock()
	idx := m.nextACL
	m.nextACL++
	m.acls[idx] = &vppacl.ACLDetails{ACLIndex: idx, Tag: tag}
	return idx
}

// Has reports whether an ACL with this index exists (F-rpf-adl-pbr's ABF policy validation).
func (m *ACLModel) Has(idx uint32) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	_, ok := m.acls[idx]
	return ok
...
func init() { RegisterExtension("acl", (*VPP).installACL) }

// installACL registers the acl plugin handlers on v (called through the TD-23 RegisterExtension registry).
func (v *VPP) installACL() {
	...
	v.On("acl_add_replace", func(req api.Message) ([]api.Message, error) {
		...
		if len(r.Tag) >= 64 {
			return reply(&vppacl.ACLAddReplaceReply{Retval: RetvalACLInvalidValue})
		}
		for _, rule := range r.R {
			...
			if rule.SrcportOrIcmptypeFirst > rule.SrcportOrIcmptypeLast || rule.DstportOrIcmpcodeFirst > rule.DstportOrIcmpcodeLast {
				return reply(&vppacl.ACLAddReplaceReply{Retval: RetvalACLInvalidValue2})
			}
		}
		idx := r.ACLIndex
		if idx == aclWildcard {
	...
	v.On("acl_interface_set_acl_list", func(req api.Message) ([]api.Message, error) {
		...
		if int(r.NInput) > len(r.Acls) {
			return reply(&vppacl.ACLInterfaceSetACLListReply{Retval: RetvalACLInvalidValue})
		}
	...
	v.On("cli_inband", func(req api.Message) ([]api.Message, error) {
		...
		if r.Cmd != "show acl-plugin tables mask" {
			return reply(&vlib.CliInbandReply{Retval: RetvalACLInvalidValue, Reply: "fake vpp: only the acl counters flag is modelled"})
		}
		m.mu.Lock()
		defer m.mu.Unlock()
		on := 0
