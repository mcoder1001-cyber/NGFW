...
| `/usr/sbin/nft` | nftables (F-host-acl-nftables; `nftables.Binaries()` = nft only) | host firewall: validate a staged rendering, load it as one transaction, read the table back. Only `table inet vrx` (tests `vrx_<prefix>`, inside the slot netns entered by `setns` on a locked thread — never `ip netns exec`) is ever loaded; never `flush ruleset`, never another table. On the shared host only `-c` and `list` touch the root netns | `nft -c -f <staged rules file>` · `nft -f <rules file>` · `nft -j list table inet <vrx\|vrx_<prefix>>` | F-host-acl-nftables |
| `/usr/bin/ip` | nftables test harness `nftables/nftest` only — **test-only**; never in `nftables.Binaries()` | create/delete `ns-<prefix>-hacl` / `ns-<prefix>-hpeer` and the veth pair `<prefix>h0`↔`<prefix>p0` on 10.<slot>.77.0/24; sockets and nft calls enter the namespaces by `setns`, not `ip netns exec` | `ip netns add\|delete <ns>`, `ip link add <prefix>h0 netns <ns> type veth peer name <prefix>p0 netns <ns>`, `ip -n <ns> addr add\|link set …` | F-host-acl-nftables |
