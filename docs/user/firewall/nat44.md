...
`nat/sessions/<protocol>/<inside>:<port>/<external>:<port>/<vrf>`). A filter on the outside or external address, a port
or the protocol makes the agent look through the sessions; on very large tables it stops at 200 000 sessions and
answers `truncated: true` (the total is then a lower bound).
