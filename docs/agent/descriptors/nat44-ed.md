...
| `nat44-ed.vrf-table` | `<table>` | `nat44_ed_add_del_vrf_table` + `nat44_ed_add_del_vrf_route` per route | route set in place | `nat44_ed_vrf_tables_dump` (v1) | `nat44-ed.enable`, optional `vrf/<table>`, `vrf/<route>` | Ownership by table range (slot `N000–N999`). **VPP quirk:** `nat44_ed_vrf_tables_v2_dump` answers with v1 `nat44_ed_vrf_tables_details` messages on 26.06 (govpp rejects them) → the v1 dump is used; same fields. |
