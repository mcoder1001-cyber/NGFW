...
|---|---|---|---|---|---|---|
| `cnat.translation` | `<vip>/<proto>/<port>` | `cnat_translation_update` (VIP endpoint, ≥1 path, lb type) → Meta `{ID}` / `cnat_translation_del(id)` | in place: `cnat_translation_update` on the same (vip, port, proto) keeps the id | `cnat_translation_dump`, owned when the VIP address is in the owner's scope | optional `cnat.snat-addresses/global` | Address endpoints only (sw_if_index `~0`); interface-resolved endpoints are not modelled. Path flags are masked to `CNAT_EPT_NO_NAT`, because the other bits in the dump are internal tracker state. **Not modelled:** `flags` (alloc-port / no-return-session / no-client), `is_real_ip` and `flow_hash_config` are not in the details (write-only fields). VPP's defaults are sent (flags 0, `is_real_ip` 0 = exclusive VIP, which is also the CLI default); Q6. |
...
| `cnat.snat-policy` | `global` | `cnat_set_snat_policy` (none / if-pfx / k8s / dnat); Delete sets `none` | in place | **no getter** → `ErrRetrieveUnsupported`, write-only (D-063) | `cnat.snat-addresses/global` | Guarded: sent only after `cnat_get_snat_addresses` confirms a default entry exists (see hazards). |
| `cnat.snat-interface` | `<interface>/<table>` | `cnat_snat_policy_add_del_if` (table include-v4 / include-v6 / pod / host) | recreate | **no dump** → `ErrRetrieveUnsupported`, write-only (D-063). Delete without a Meta resolves the interface by name. | `cnat.snat-addresses/global`, `interface/<name>` | FEATURE_DISABLED on add → `ErrNoSnatDefault`. |
| `cnat.snat-exclude-prefix` | `<prefix>` | `cnat_snat_policy_add_del_exclude_pfx` | recreate | **no dump** → `ErrRetrieveUnsupported`, write-only (D-063) | `cnat.snat-addresses/global` | Guarded like snat-policy. |
...
**Write-only objects (D-063):** snat-policy, snat-interface and snat-exclude-prefix have no VPP getter, so their
Retrieve returns `ErrRetrieveUnsupported`. The reconciler re-applies them on every resync. That is idempotent for the
policy and the interface bitmap. For an excluded prefix, VPP bumps a per-prefix-length refcount, which only affects
the search order; the entry itself is a bihash add and disappears with the default entry. Open question Q6 in
`DF-3-questions.md`.

Tests: `cnat_test.go` runs on the fake. It covers create, idempotent re-apply, in-place translation update, convergence
of a fresh process without any cache, the write-only contract of the snat-* objects, foreign VIPs and a foreign SNAT entry, the crash guards (the fake fails the test if a guarded
