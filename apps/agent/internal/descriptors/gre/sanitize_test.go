...
func TestTunnelSanitizesReusedIndex(t *testing.T) {
	...
	m.PoisonActive(1, 2, 3, 4)
	if _, err := d.Create(ctx, &gre.Tunnel{Instance: 1101, Src: "10.11.1.1", Dst: "10.11.1.3", Type: gre.TunnelType_L3}); !errors.Is(err, ifsanitize.ErrActiveStaleBinding) {
		t.Fatalf("err = %v", err)
	}
	if len(f.tunnels) != 1 {
