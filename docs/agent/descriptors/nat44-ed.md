...
|---|---|---|---|---|---|---|
| `nat44-ed.enable` | `global` | `nat44_ed_plugin_enable_disable` (enable / disable) | owner: disable+enable only while the plugin is empty (`ErrNotEmpty`); non-owner: requirement check (D-071) | `nat44_show_running_config` (`sessions == 0` ⇔ disabled; VPP zeroes rconfig on disable) | optional `vrf/<inside>`, `vrf/<outside>` | **Global singleton (D-071).** Owner Create reads first: already enabled with the same config ⇒ converged (no call); different config ⇒ error; refused while nat44-ei is on (`ErrOtherVariant`). Non-owner: `ErrGlobalNotSet` / `ErrGlobalMismatch`. `sessions: 0` normalises to VPP's default 64512 (`DefaultSessions`). `static-mapping-only`/`connection-tracking` are `UNSUPPORTED` in 26.06 and not modelled. Retval `FEATURE_ALREADY_ENABLED/DISABLED` is idempotent success. |
| `nat44-ed.timeouts` | `global` | `nat_set_timeouts` / Delete restores VPP defaults 300/7440/240/60 | in place (`nat_set_timeouts`) | `nat44_show_running_config.timeouts` (only while enabled) | `nat44-ed.enable` | Singleton. **Presence = non-default values**: Retrieve reports the object only when the timeouts differ from VPP defaults (else a desired state without it would reset them every pass). `nat44_ed_set_timeouts` does not exist in 26.06. |
| `nat44-ed.forwarding` | `global` | `nat44_forwarding_enable_disable` / Delete = disable | — (empty spec) | `nat44_show_running_config.forwarding_enabled` | `nat44-ed.enable` | Singleton; **presence = enabled** (spec has no fields). |
...
| `nat44-ed.vrf-table` | `<table>` | `nat44_ed_add_del_vrf_table` + `nat44_ed_add_del_vrf_route` per route | route set in place | `nat44_ed_vrf_tables_v2_dump` on a raw stream, accepting v1 **or** v2 details | `nat44-ed.enable`, optional `vrf/<table>`, `vrf/<route>` | Ownership by table range (slot `N000–N999`). **VPP quirk:** on 26.06 the v2 dump answers with v1 `nat44_ed_vrf_tables_details` messages and the v1 dump answers nothing; the generated v2 client rejects the v1 type, hence the raw stream (questions file Q4). |
