...
const fetchRevisions: FetchPage<RevisionMeta> = async ({ page, pageSize }, signal) => {
  const { data } = await call(api.GET('/api/v1/config/revisions', { params: { query: { limit: pageSize, offset: page * pageSize } }, signal }));
...
function RevisionDiff({ from, to }: { from: number; to: number }) {
  ...
  const changes = useMemo(() => (b.data && (from === 0 || a.data) ? diff(from === 0 ? {} : a.data!.payload, b.data.payload) : null), [a.data, b.data, from]);
  const error = a.error ?? b.error;
  return (
    <Paper variant="outlined" sx={{ p: 2 }} component="section" aria-label={t('compare.result', { from, to })} data-testid="revision-diff">
      <Typography component="h3" variant="subtitle1" sx={{ mb: 1 }}>
        {from === 0 ? t('compare.initial', { to }) : t('compare.result', { from, to })}
      </Typography>
      {error ? <ProblemAlert error={error} /> : changes === null ? <LinearProgress aria-label={t('loading')} /> : <DiffView changes={changes} />}
    </Paper>
  );
}
...
export function RevisionsPage() {
  ...
  const columns = useMemo<GridColDef<RevisionMeta>[]>(
    () => [
      ...
      { field: 'createdAt', headerName: t('col.createdAt'), width: 190, sortable: false, valueFormatter: (v: string) => fmt.dateTime(v) },
      { field: 'author', headerName: t('col.author'), width: 120, sortable: false, valueFormatter: (v: string | null) => v ?? t('col.system') },
      {
        field: 'kind',
        headerName: t('col.kind'),
        width: 110,
        sortable: false,
        renderCell: ({ value }) => <Chip size="small" variant="outlined" label={t(`kind.${String(value)}`, { defaultValue: String(value) })} />,
      },
      { field: 'comment', headerName: t('col.comment'), flex: 1, minWidth: 160, sortable: false },
      {
        ...
        renderCell: ({ row }) => (
          <Stack direction="row" gap={1} alignItems="center" sx={{ blockSize: '100%' }}>
            <Button size="small" startIcon={<CompareArrowsIcon />} onClick={() => setCmp({ from: row.parentId ?? 0, to: row.id })}>
              {t('changes')}
            </Button>
  ...
  const canCompare = Number.isInteger(parsedFrom) && Number.isInteger(parsedTo) && parsedFrom >= 1 && parsedTo >= 1 && parsedFrom !== parsedTo;
  ...
  return (
    <PageHeader title={t('title')}>
      ...
      <Stack component="form" direction="row" gap={1} alignItems="center" flexWrap="wrap" sx={{ mb: 2 }} onSubmit={(e) => {
        e.preventDefault();
        if (canCompare) setCmp({ from: parsedFrom, to: parsedTo });
      }}>
        <Typography>{t('compare.label')}</Typography>
        <TextField size="small" type="number" label={t('compare.from')} value={fromInput} onChange={(e) => setFromInput(e.target.value)} sx={{ inlineSize: 120 }} slotProps={{ htmlInput: REV_INPUT }} />
        <TextField size="small" type="number" label={t('compare.to')} value={toInput} onChange={(e) => setToInput(e.target.value)} sx={{ inlineSize: 120 }} slotProps={{ htmlInput: REV_INPUT }} />
        <Button type="submit" variant="outlined" disabled={!canCompare} startIcon={<CompareArrowsIcon />}>
          {t('compare.submit')}
        </Button>
      </Stack>
