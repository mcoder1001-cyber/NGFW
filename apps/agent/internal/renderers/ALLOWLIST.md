...
| `/usr/bin/journalctl` | log explorer (F-unbound-chrony-syslog, `internal/actions/unbound-chrony-syslog`; its own runner `ucsaction.NewRunner()`, journalctl only, 16 MiB output bound) | read one bounded page of the local journal, newest first; filters are validated enum values and numbers — the free-text query is matched in the agent, never passed (no `--grep`) | `journalctl --no-pager --quiet --output=json --output-fields=MESSAGE,PRIORITY,SYSLOG_FACILITY,SYSLOG_IDENTIFIER,_COMM,_PID,_HOSTNAME,_SYSTEMD_UNIT --reverse --lines=5000 --since=@<unix seconds> [--priority=<0-7>] [--facility=<syslog facility name>]` | F-unbound-chrony-syslog |
