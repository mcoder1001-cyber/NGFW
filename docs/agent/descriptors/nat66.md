...
|---|---|---|---|---|---|---|
| `nat66.enable` | `global` | `nat66_plugin_enable_disable` (`outside_vrf`; already-enabled retval tolerated) | `ErrRecreate` (refused with `ErrForeignObjects` when foreign objects exist) | `ErrRetrieveUnsupported` — **write-only** (D-063) | optional `vrf/<outside>` | No getter for "enabled" or `outside_vrf`. The re-apply on every resync is idempotent, but an enable on an already enabled plugin keeps its old VRF: a VRF change needs the singleton removed and re-added. |
