...
type State struct {
	...
	// UnlistedSAs counts IKE_SAs charon reports in stats but that belong to no connection
	// loaded over VICI (e.g. SAs of a connection unloaded by someone else); 0 when in sync.
...
// JSON tags; D-055 stand-in until P11 adds an IPsec state message), redacted.
...
func (r *Renderer) stateStruct(st *State) (*structpb.Struct, error) {
	...
	if err := json.Unmarshal([]byte(r.secrets.redact(string(raw))), &m); err != nil {
