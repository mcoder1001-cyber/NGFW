 ...
 *
 * The arpFlush Action case is registered with the fake agent's TD-23 action dispatcher (registerActionHandler); formerly
 * this module's `action` replaced the generic one (the spread comes after it in
...
// The fake agent the arpFlush handler answers for: the one whose service was built last (API tests run one
// fake agent at a time). The dispatcher calls handlers with the call only, and records the call itself.
let arpFlushFake: FakeAgent | undefined;
const arpFlushAction: ActionHandler = (call) => {
  if (arpFlushFake === undefined) {
    call.emit('error', grpcError(status.UNAVAILABLE, 'fake agent: no neighbours fake'));
    return;
  }
  arpFlush(arpFlushFake, call);
};
...
export function neighborsRaFake(fake: FakeAgent): {
  ...
  arpFlushFake = fake;
  registerActionHandler('arpFlush', arpFlushAction); // TD-23: the dispatcher routes Action by its oneof case
  return {
