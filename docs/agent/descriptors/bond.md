...
| `bond.member` | `bond.member/<bond id>/<member id>` | bond key, member interface key | `bond_add_member` (`is_passive`, `is_long_timeout`), `bond_detach_member`; Retrieve `sw_member_interface_dump` per owned bond | ErrRecreate | Member must be an owned hardware interface (tap, af-packet, memif; DPDK NICs do not exist on the dev host). Meta `{SwIfIndex, Bond}`. |
