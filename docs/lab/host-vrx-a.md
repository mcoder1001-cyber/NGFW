...
| Item | Fact |
|---|---|
| OS / kernel | Ubuntu 26.04.1 LTS, **7.0.0-31-generic** (re-verified 2026-09-24 14:45), VMware VM (VMXNET3), **32 vCPU, 94 GB RAM, 2 vNUMA nodes**, 197 GB root. VMware balloon holds 1 GiB and first-touch of fresh memory is slow (VPP audit D-108) |
...
| Hugepages | configured 1024 × 2 MB via `/etc/sysctl.d/80-vpp.conf`, **but only 555 allocated this boot**: systemd-sysctl times out writing vm.nr_hugepages (volatile; fix proposed in PENDING-vpp-host-hardening option B) |
...
| af_packet rings | VPP default TX ring = 66 KiB × 1024 = 66 MiB zeroed on the main thread → 40 s–5.5 min API stalls here; rig and agent use `tx-size 2048 tx-per-block 256` (D-108) |
