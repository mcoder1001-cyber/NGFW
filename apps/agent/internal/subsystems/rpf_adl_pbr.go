...
func init() { Domains[servicesDomain] = append(Domains[servicesDomain], rpfAdlPbrAutoSdl) }
...
func (w *Wiring) registerRpfAdlPbr(r scheduler.Registry) error {
	...
	// ABF policy ids come from the agent's id range (TD-8b, seams.go), fail-closed like F-qos-flat's egress maps:
	// without a range the family owns no policy id and says so; the product agent always has one.
	rng, err := w.IDRange()
	var ids *df2.IDRange
	if errors.Is(err, ErrNoIDRange) {
		w.env.Log.Warn("rpf-adl-pbr: no VPP id range — ABF policies (routing.pbr) cannot be created", "err", err)
	} else if err != nil {
		return fmt.Errorf("rpf-adl-pbr: %w", err)
	} else {
		ids = rng.DF2()
	}
...
// Persistent reports whether the name↔id records survive an agent restart: they do when backed by a file
// (openPbrNames with a StateDir path); the in-memory default (empty path) is for tests. (TD-11b, dfkit/persist.)
func (s *pbrNames) Persistent() bool { return s.path != "" }

// CheckPersistent declares how pbr.policy records ownership for the TD-11b guard: its name↔id records must be in
// a store that survives an agent restart (the product wiring passes a StateDir file path).
func (s *pbrNames) CheckPersistent() error {
	return persist.Require("pbr.policy: name↔id records", s)
}
...
// RecordsNoOwnership declares (TD-11b): the ACL bridge is an observe-only alias resolver, it owns no VPP object.
func (*aclRefs) RecordsNoOwnership() {}
