...
| `memif.memif` | `memif.memif/<name>` | `memif.socket/<socket id>` | `memif_create_v2`, `memif_delete`, `sw_interface_tag_add_del`; Retrieve `memif_dump` (+ owner tag from `sw_interface_dump`) | ErrRecreate | role master / slave, mode ethernet / ip / punt-inject, `id`, zero-copy. **Not modelled** (cannot round-trip): ring_size / buffer_size (`memif_details` reports the values negotiated with the peer, 0 before it connects), queue counts, secret, hw address (use `interface.mac-address`). VPP defaults apply (1024 / 2048). |
