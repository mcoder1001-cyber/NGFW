...
| `….v3Users.<name>.view` *(F-snmp)* | `-V <view>` | defined view |
...
| `services.snmp.monitors.disks[i].{path,minPercent}` *(F-snmp)* | `disk <path> <n>%` | absolute clean path `[A-Za-z0-9_./-]`, 1–99 (default 10), ≤ 16 |
| `services.snmp.monitors.load.{max1,max5,max15}` *(F-snmp)* | `load <1> <5> <15>` | 1–1000, all three required |
| `services.snmp.description` | not rendered | — |

Retrieve: `configured`, `reachable`, `endpoint`, `credential`
...
stays rejected: binding snmpd to a VRF needs a Linux VRF (linux-cp), not cheap in this build.
