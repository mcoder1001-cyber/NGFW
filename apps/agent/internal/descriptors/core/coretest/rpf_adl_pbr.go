...
func init() { extensions = append(extensions, (*VPP).installRpfAdlPbr) }
...
func (v *VPP) installRpfAdlPbr() {
	...
	v.On("feature_is_enabled", func(msg api.Message) ([]api.Message, error) {
		req := msg.(*featureapi.FeatureIsEnabled)
		v.mu.Lock()
		defer v.mu.Unlock()
		switch {
		case req.ArcName == "device-input" && req.FeatureName == "adl-input":
			return reply(&featureapi.FeatureIsEnabledReply{IsEnabled: m.AdlInput[uint32(req.SwIfIndex)] > 0})
		case req.ArcName == "device-input" && req.FeatureName == "ethernet-input":
			return reply(&featureapi.FeatureIsEnabledReply{})
		default:
			return reply(&featureapi.FeatureIsEnabledReply{IsEnabled: true}) // V23 (a): an error reads as true
		}
	})
