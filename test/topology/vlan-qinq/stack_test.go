...
// The stack under test (copied from P08's test/topology/interfaces, the pattern this module follows): the real vrx-agent
// binary (built from this tree, owner = VRX_TEST_PREFIX) and the real vrx-api (apps/api/dist, slot port) on a throwaway
// slot database, the host VPP, the af_packet veth/netns rig. Every process is started by this test and stopped by PID;
// every object carries the slot prefix.
