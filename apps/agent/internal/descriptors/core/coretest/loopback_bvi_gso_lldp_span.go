...
//
// feature_is_enabled is one handler for the whole model: this file answers the gso-ip4 read-back and
...
const (
	...
	RetvalInvalidInterface           int32 = -11
)
...
func (v *VPP) installLoopbackBviGsoLldpSpan() {
	...
	v.On("feature_is_enabled", func(req api.Message) ([]api.Message, error) {
		...
		if r.ArcName == "ip4-output" && r.FeatureName == "gso-ip4" {
			m := v.lbgs()
			v.lbgsGCLocked(m)
			return reply(&feature.FeatureIsEnabledReply{IsEnabled: m.gso[idx] > 0 || !m.gsoReached[idx]})
		}
		// F-bridge-l2's answer (coretest/bridge_l2.go: mactime on device-input)
		m := v.l2()
		v.gcLocked(m)
	...
	v.On("sw_interface_span_enable_disable", func(req api.Message) ([]api.Message, error) {
		...
		if from == to {
			return reply(&spanapi.SwInterfaceSpanEnableDisableReply{Retval: RetvalInvalidInterface})
		}
