...
| `routing.static[i].nextHops[j].address` | `… <prefix> <address>` | `net/netip`, no zone, same family as the prefix |
| `routing.static[i].nextHops[j].interface` | `… <prefix> [<address>] <linux name>` | mapped + Linux name rule; unmapped = error |
| `routing.static[i].nextHops[j].blackhole` *(stand-in, D-055)* | `… <prefix> blackhole` | no address/interface allowed with it |
...
| `runningConfig` | `show running-config`, normalised (no banner, comments, blank lines, `frr version`, `end`) — for drift |
| `vrfs` | `show vrf` (text; 10.7 has no JSON variant) → `[{name, active, id, table}]` |
| `ipv4Routes` / `ipv6Routes` | `show ip route vrf all json` / `show ipv6 route vrf all json` |
...
|---|---|---|
| `routes` | `ipv4/<vrf>`, `ipv6/<vrf>` → RIB entry count | `EVENT_KIND_UNSPECIFIED`, attributes `source=frr poller key old new` |
