...
// raw posts a non-JSON body (CSV) with its content type.
func (a *api) raw(method, path, contentType, body string) resp {
	a.t.Helper()
	...
	r := resp{status: res.StatusCode, raw: string(b)}
	_ = json.Unmarshal(b, &r.body)
	return r
}
