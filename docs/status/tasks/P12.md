...
(Timing note, review L2: the "after N ms" figures start when the preceding `apply` (or the peer's apply) returned and are
bounded by the 100–250 ms poll step; they measure time-to-observed after the reload, not from the triggering event.
Acceptance still holds: link-down → BGP < 1 s, withdraw < 5 s.)
...
### 7. Routes present in the VPP FIB (linux-nl) — **DEFERRED: row P12-fib-proof** (manager decision on review H3)

Not proven by P12. Every count above is FRR's RIB. On the shared VPP, linux_nl's single netlink socket listens in the netns
that was the lcp default netns when the first pair of the whole VPP was created — root — and a pair is heard only if its
netns equals that default; FRR's netns `ns-w8-frr` is not heard. The shared-VPP workaround (T2: a global default-netns
window) is dropped (unsafe: review H3/Q1). The proof moves to row **P12-fib-proof**, gated on **LAB-vpp-per-slot** (a VPP of
the slot's own; parked on the product owner's PENDING-vpp-host-hardening). P12 merges with this acceptance item open.

**Test design for P12-fib-proof** — runs the existing test unchanged, only the environment differs:

| | |
|---|---|
| command | `eval "$(tools/lab env <N>)"; VRX_P12_FIB=private VRX_VPP_API_SOCKET=<private api.sock> test/topology/bgp/run.sh` (the stats socket likewise if the private VPP has its own) |
| private VPP (LAB-vpp-per-slot) | VPP 26.06 with `linux_cp` + `linux_nl` enabled and **`linux-cp { default netns ns-<p>-frr }`** in its startup.conf; nothing else on it creates pairs; its FIB table 0 is its own |
| netns | `ns-<p>-frr` must exist before the first pair: the test's frrtest harness creates it (step 0) before the agent's first apply; VPP opens linux_nl's socket there with the first pair |
| preconditions (the test refuses otherwise) | `VRX_VPP_API_SOCKET` ≠ `/run/vpp/api.sock`; `lcp_default_ns_get` = `ns-<p>-frr`; `ListRoutes(table 0, 10.<N>.0.0/16, source lcp-rt-dynamic)` total = 0 |
| pairs | `interfaces.host-<p>{l0,w0}.lcp` **without** `netns` (the default netns — the only one linux-nl hears; review M4) |
| VPP checks (every step, beside FRR's RIB) | `ListRoutes(vrf default, family ipv4, prefix 10.<N>.0.0/16, source lcp-rt-dynamic).total`: commit **200** · route map denying half **100** · peer 1 withdraws **50** within 5 s · announces again 100 · agent restart with both pairs deleted behind its back → 100 **and** a second withdraw/announce cycle 50 → 100 (a deleted pair flushes no route, so only a withdrawal proves linux-nl hears the recreated pairs) · link down / up · rollback of the BGP config (before any pair is deleted) **0** |
| evidence to paste | `vppctl -s <private cli> show ip fib summary` + `show ip fib 10.<N>.64.0/25` (path via `host-<p>l0`, source `lcp-rt-dynamic`) at 200/100/0; `show lcp` (`lcp default netns ns-<p>-frr`); NRestarts of the private VPP before/after |
| cleanup | the test's own: peers down, the agent deletes pairs/af_packet (veths down first, D-101), harness stop; the private VPP's table 0 holds no `lcp-rt-dynamic` route of 10.<N>/16 (asserted after the rollback) |
| never | any VPP-global change on the shared VPP; the test itself sets no global on the private one either (the netns comes from startup.conf) |
...
The gate run at 04:35 stopped at **gitleaks**: one `generic-api-key` hit on a test literal in commit 6dff01fc (Q16, a false
positive fixed forward; the D-112 squash does **not** drop it by itself — Q16 quoted the literal; reworded in fix round 1, H2). `ci.sh` stops at the first failed step, so the remaining steps were
run by hand on HEAD (05:0x):

| step | result |
|---|---|
| generate + generated-output gate | clean (ci.sh 04:35: `clean: packages/proto/gen apps/agent/gen packages/schema/dist packages/api-client/src/generated`) |
| contract guard | passed (contract commits `contract(schema)`, `contract(proto)`, `contract(api-client)`) |
| forbidden patterns | ok (ci.sh 04:35) |
| `pnpm turbo run lint typecheck test build` | `Tasks: 30 successful, 30 total` |
| `make -C apps/agent lint build` | `0 issues`, build ok |
| `apps/agent` `go test ./...` | red **only** from F-vrf-static-ecmp's svs descriptors under main's TD-11b guard (Q15) and F-vrf's `TestSvsRangeFromSlot`; with the local svs declaration overlay: every package ok except `TestSvsRangeFromSlot` |
| `make -C apps/cli lint test build` | `0 issues`, 7 packages ok (operations table regenerated for `Bgp_state`) |
| `test/` Go modules (unit mode) | smoke, interfaces, vrf-static-ecmp: ok |


## Fix round 1 (review 7931c596, APPROVE WITH CHANGES)

Every code fix carries a test that fails on the code before it (checked by running the new test against the old code).

| finding | fix | commit | test (fails on the old code) |
|---|---|---|---|
| H1 frr.config hard-depends on every pair | `frrConfigDescriptor.Dependencies` returns nil; `desired.FRRDependencies` removed. The pairs do not order FRR: an `update-source`/`interface` line that names a missing tap is rendered and FRR waits for the device | 7bc233b6 | `subsystems.TestFRRStageSurvivesPairChanges`: real scheduler, two pairs; removing one pair and recreating the other (tap→tun) gives frr.config ops exactly `[update]` and never a framework-only frr.conf. Old code: `recreate` |
| H2 Q16 quoted the gitleaks literal | Q16 reworded; the literal is gone from P12's own files | 2fecb868 | gitleaks on the squash diff, below |
| H3 T2 changed the shared VPP's lcp default netns | T2 removed (`VRX_P12_LINUX_NL`, `enableFIBProof`). The VPP FIB proof is **deferred, row P12-fib-proof** (gated on LAB-vpp-per-slot); its test design is in the acceptance section above and runs unchanged with `VRX_P12_FIB=private`, which refuses `/run/vpp/api.sock` and requires the private VPP's default netns = `ns-<p>-frr` | 941adbd1, 0a3c3a2e | — (test path removed; `go vet`/build of the host test) |
| M2 Zod vs Go description rules | Zod: `routing.bgp-frr-description` (≤ 80, printable ASCII, no leading/trailing blank, no leading `!`/`#`, no `\|`, no double blank) on neighbours, peer groups, prefix lists, route-map entries; `routing.bgp-route-map-seq` (≤ 65535) at the entry. Go: interface descriptions are no longer rendered into FRR (a Persian interface description passes both); render errors are `policy.FieldError`s with a path, reported at the field's pointer | 4ccea14c, 7e24b235 (contract) | `agent.TestP12RenderErrorsPointAtTheField` (6 cases, old code: every error at `/routing`, route-map index by seq); `semantic/bgp.test.ts` `frr-description`, `route-map-seq` (old code: 2 fail) |
| M3 refetch storm / RoutingState walks | web: routing events invalidate `/state/bgp` at most every 30 s (`eventRefetchDue`), the 30 s poll and the Refresh button stay. api: `/state/routes` calls RoutingState only with `proto`. agent: one FRR+VPP walk in flight (`FRR.stateSlot`), a second call waits ≤ 3 s then gets `UNAVAILABLE` (`ErrStateBusy`); the linux-cp pair dump runs inside the same walk | 9ad56d0b, b30b439d | `subsystems.TestStateIsSerialised`; `bgp/routes.test.ts` (3; old code: agent called without `proto`); `bgp/model.test.ts` "at most every 30 s" |
| M4 a pair in a netns other than the lcp default is ignored by linux-nl | build-time warning `routing.bgp-lcp-netns` at `/interfaces/<name>/lcp/netns`; documented in `docs/agent/descriptors/lcp.md`, `docs/user/routing/bgp.md`, the form's help (en/fa) | ee9290a9 | `agent.TestP12ProjectionWithoutFRR`: exactly one warning, at `/interfaces/loop821/lcp/netns` |
| L1 Q8 said 1000 lookups | Q8: ≤ 100 per page | ee9290a9 | — |
| L2 timing | note in the acceptance section | 0a3c3a2e | — |
| L3/L4 | notes in `frr-bgp.md`; `tapGatedPairs` comment; passwordRef help says it is refused until PENDING-secret-channel | 83fb4933 | — |
| M1 stale F-vrf-static-ecmp copy | **waiting**: F-vrf lands first; then `merge main`, take F-vrf's files, re-apply only P12's `proto` query, `@ApiQuery` and the BGP nav line | — | — |

### gitleaks on the squash diff (H2)

`git diff $(git merge-base main HEAD) HEAD | gitleaks stdin --config .github/gitleaks.toml`:

```
full squash diff:                 WRN leaks found: 1  (exit 1) — docs/status/tasks/P12-review.md:27, the review's H2 row quoting the literal
squash diff without P12-review.md: INF no leaks found (exit 0)
```

P12's own files are clean. The one remaining hit is the reviewer's file (7931c596); my edit of it was refused
(permission), so **the reviewer or the manager must reword line 27** (e.g. "the literal `Key:` + a quoted RIB key")
before the squash, or the merge gate fails on it. The commit-history scan in `ci.sh` also reports the two old P12
commits (2b14358f Q16, 6dff01fc `frr_test.go`); the D-112 squash removes those, since neither line is in the final tree.

### CI after fix round 1

`TMPDIR=/tmp/g-w8 tools/ci.sh --base main` at 09:56 on fcc2dbd1 (`/root/ngfw-wt/logs/ci/P12-20260925-095606-667842`):
contract guard ok, generate + generated-output gate `clean`, then **stopped at gitleaks (history)** with 3
`generic-api-key` hits: `P12-questions.md:165` @2b14358f and `frr_test.go:183` @6dff01fc (old P12 commits, gone from
the tree, dropped by the D-112 squash) and `P12-review.md:27` @7931c596 (the review's own quote, see above). The
remaining steps were run by hand on the same HEAD:

| step | result |
|---|---|
| `pnpm turbo run lint typecheck test build` | `Tasks: 30 successful, 30 total` (schema 1236 passed) |
| `make -C apps/agent lint build` | `0 issues`, build ok |
| `apps/agent` `go test -race` (subsystems, desired, renderers/frr, descriptors/lcp, agent) | ok with the local svs overlay (Q15), except F-vrf-static-ecmp's `TestSvsRangeFromSlot` (not P12; unchanged since round 0) · `go vet -tags integration` of the host test ok |
| `make -C apps/cli lint test build` | `0 issues`, 7 packages ok |
| `test/` Go modules (unit mode) | smoke, interfaces, vrf-static-ecmp: ok · `run.sh` syntax ok |
| host runs | none (af_packet creates are blocked until TD-25) |
...
| D-071 no `lcp default netns` change from a slot | nothing in P12 changes it: the T2 path is gone (fix round 1, H3); the FIB mode `VRX_P12_FIB=private` refuses the shared VPP and reads the default netns from the private VPP's startup.conf |
...
| D-132 | the BGP screen polls `/state/bgp` at 30 s, Refresh button; WS events invalidate at most every 30 s (fix round 1, M3); the agent runs one FRR+VPP walk at a time and answers UNAVAILABLE after 3 s |
...
Q1 linux-nl test plan (T1 on the shared VPP; the VPP FIB proof = row P12-fib-proof on a private VPP — no T2) · Q2 LCP leaf on the interface (22) · Q3 renderer stage =
...
Q1 (decided: no T2; FIB proof = row P12-fib-proof) · Q13 (the VPP crash was dns_plugin, resolved) · Q14 (CLI) · Q15 (TD-11b vs the svs
