...
// aclRecSink records what a builder emits.
type aclRecSink struct {
	kvs    []scheduler.KV
	ptrs   map[scheduler.Key]string
	issues []string
}

func newRecSink() *aclRecSink { return &aclRecSink{ptrs: map[scheduler.Key]string{}} }

func (s *aclRecSink) Add(k scheduler.Key, v proto.Message, pointer string) {
	s.kvs = append(s.kvs, scheduler.KV{Key: k, Value: v})
	s.ptrs[k] = pointer
}
func (s *aclRecSink) Errorf(pointer, rule, format string, a ...any) {
	s.issues = append(s.issues, "E "+pointer+" "+rule+" "+fmt.Sprintf(format, a...))
}
func (s *aclRecSink) Warnf(pointer, rule, format string, a ...any) {
	s.issues = append(s.issues, "W "+pointer+" "+rule+" "+fmt.Sprintf(format, a...))
}

func (s *aclRecSink) value(t *testing.T, k scheduler.Key) proto.Message {
	t.Helper()
