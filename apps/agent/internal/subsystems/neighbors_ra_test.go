...
// TestNeighborsRaIDRange: proxy-ARP tables come from the agent's id range (TD-8b, Wiring.IDRange); without one the
// family fails closed — registration still succeeds (a warning, as F-qos-flat's egress maps), owning no table.
func TestNeighborsRaIDRange(t *testing.T) {
	reg, _ := registerNRA(t, coretest.New(), false, nil) // Env without IDs: ErrNoIDRange
	if _, ok := reg.Get(arp.RangeName); !ok {
		t.Fatal("arp.range not registered without an id range")
	}
}
