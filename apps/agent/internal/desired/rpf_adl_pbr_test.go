...
func vrfID(n string) (uint32, bool) {
	if n == "" {
		return 0, true
	}
	id, ok := vrfs[n]
...
var all = map[string]bool{"interfaces": true, "vrfs": true, "routing": true, "services": true}
...
func TestRpfAdlPbrProjection(t *testing.T) {
	...
	s := newSink()
	RpfAdlPbr(s, ds, all, vrfID, RpfAdlPbrEnv{PolicyIDs: &df2.IDRange{Lo: 3000, Hi: 3999}, AutoSdl: true})
	...
	s = newSink()
	RpfAdlPbr(s, ds, map[string]bool{"routing": true}, vrfID, RpfAdlPbrEnv{})
	...
	s = newSink()
	RpfAdlPbr(s, ds, map[string]bool{"services": true}, vrfID, RpfAdlPbrEnv{})
...
func TestRpfAdlPbrProjectionErrors(t *testing.T) {
	...
	s := newSink()
	RpfAdlPbr(s, ds, all, vrfID, RpfAdlPbrEnv{})
	sort.Strings(s.issues)
	want := []string{
...
func TestRpfAdlPbrAssemble(t *testing.T) {
	...
	RpfAdlPbrAssemble(ds, kvs, all, func(id uint32) string { return names[id] }, stored)
	want := parse(t, `{
	  "interfaces": {
	...
	RpfAdlPbrAssemble(empty, nil, all, func(uint32) string { return "default" }, nil)
