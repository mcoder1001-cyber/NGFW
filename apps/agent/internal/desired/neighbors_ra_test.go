...
func (s *nraSink) Add(k scheduler.Key, v proto.Message, pointer string) {
	if s.ptrs == nil {
...
func (s *nraSink) Errorf(pointer, rule, format string, a ...any) {
	s.issues = append(s.issues, "E "+pointer+" "+rule+" "+fmt.Sprintf(format, a...))
}
func (s *nraSink) Warnf(pointer, rule, format string, a ...any) {
	s.issues = append(s.issues, "W "+pointer+" "+rule+" "+fmt.Sprintf(format, a...))
}

func (s *nraSink) keys() []string {
	var out []string
