#!/usr/bin/env bash
# test/topology/bridge-l2/run.sh — F-bridge-l2 topology + restart-safety test on the host VPP (af_packet rig, no packets).
#   eval "$(tools/lab env <slot>)"; test/topology/bridge-l2/run.sh [go test args]
...
# only when missing; the test keeps its own files private in "$RUN/bl2" (0700)
...
export VRX_BL2_AGENT_BIN="$ROOT/apps/agent/bin/vrx-agent" VRX_INTEGRATION=1
cd "$HERE"
