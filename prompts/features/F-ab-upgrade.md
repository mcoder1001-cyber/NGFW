...
3. **Boot integration**: `vrx-upgrade-health.service` in the new slot — waits for vrx-agent/vrx-api/VPP healthy (agent `Health` RPC, API `/health`)
...
5. **Docs**: `docs/install/ab-upgrade.md` (bundle format, CLI, boot flow diagram, rollback, what the UI calls).

Files you own: `deploy/upgrade/**`, `docs/install/ab-upgrade.md`, `test/topology/ab-upgrade/**`. Shared files: none (P14's autoinstall and P10's
packaging are consumed read-only; needed changes there → `docs/status/tasks/F-ab-upgrade-questions.md`).
...
- [ ] `lsblk`, `losetup -a`, `/boot/grub/grubenv` of **this host** unchanged before/after (pasted); no loop devices left behind
