...
### F-neighbors-ra: ListNeighbors

`rpc ListNeighbors(ListNeighborsRequest) returns (ListNeighborsResponse)` — the live ARP/ND table, read-only. The
agent dumps `ip_neighbor_dump` **per interface and address family** for every interface it can name (its own tag or
untagged, never another owner's; never `sw_if_index = ~0`, which on a shared VPP is every slot's table), then filters
(`vrf`, `interface`, `family` ipv4|ipv6, `state` static|dynamic, `search` substring of ip/mac/interface), sorts
(`sort` interface|ip|mac|age|vrf|state, `descending`) and pages (`offset`, `limit` 0 = 100, max 1000) on its side;
`total` counts every matching row. `NeighborEntry.vrf` is the VRF name of the interface's table for the entry's family
("default" for table 0, the decimal table id for tables the agent does not name). Unknown filter values fail with
INVALID_ARGUMENT; the owner check is the usual one (§6).

### F-neighbors-ra: ActionRequest.arp_flush (4)
...
### F-object-model: FqdnObjectState

`FqdnObjectState(FqdnObjectStateRequest{names, owner}) → FqdnObjectStateResponse{objects[], owner, retrieved_at}` is the
...
- `rpc NatSessions(NatSessionsRequest) → NatSessionsResponse`: one page of this owner's NAT44-ED sessions. VPP dumps
  ...
  and `protocol` scan the owner's sessions one user at a time, keeping only the page. **Per-call caps** (every
  `nat44_user_session_v3_dump` walks the worker's whole session pool under the barrier): one call dumps at most 256
  users and looks at 200 000 sessions; hitting a cap sets `truncated` (the filtered total is then a lower bound, the
  page may be short — continue at `next_offset`). The v3 dump matches the inside address only (not the VRF): users
  that share an address in several VRFs partition that address's dump, so no session is listed twice, but the VRF
  label of such rows cannot be verified (docs/vpp-code-track.md V-new). Ownership (§6): a test slot `w<N>` sees only users in 10.N.0.0/16;
...
- `ActionRequest.nat_session_kill = 5` (`NatSessionKillAction`): `nat44_del_session` with `NAT_IS_INSIDE` for the
  ...
  addresses it**: `external_nat_*` of a twice-NAT session, `external_*` otherwise. `NatSession.external_nat_*` is
  "0.0.0.0"/0 without twice-NAT (VPP fills it only for twice-NAT sessions). Validation (`INVALID_ARGUMENT` before any output): addresses, ports,
...
### F-nat44-ei-64-66-nptv6: NAT session variants

Additive fields on F-nat44-ed-sessions' messages (after their maxima, nothing renamed) and one new enum in the
`// ----- F-nat44-ei-64-66-nptv6 -----` section. No new RPC, no new `ActionRequest` member, no `NatConfig` number.
...
- **NAT64** (`nat64_st_dump`, one stream per protocol, 255 = all): read-only, paged in the agent (offset/limit, same
  ...
  any other filter field is `INVALID_ARGUMENT`. Field mapping: `inside_*` = the IPv6 client (`il_addr`/`il_port`),
  `outside_*` = the IPv4 pool endpoint (`ol_addr`/`ol_port`), `external_*` = the IPv4 remote (`or_addr`/`r_port`),
...
`GET /api/v1/state/nat/nptv6` (the running bindings with a write-only marker; no agent RPC: npt66 has no dump).
<!-- wave-A: P11 -->
<!-- wave-A: F-wireguard -->
<!-- wave-A: P12 -->
