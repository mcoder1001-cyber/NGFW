...
| V12 | ip6-nd proxy: `ip6nd_proxy_add_del` aborted the shared VPP (interface-output) | DF-2 (Q1) | crash on a valid-looking API call — reproduction pending | proxy-ND descriptor opt-in only (`VRX_DF2_PROXY_ND=1`), not in default Register | 1–2 days |
