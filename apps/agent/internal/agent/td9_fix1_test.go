...
func TestNarrowerSupersedeOfAnOwedRevertStillResyncs(t *testing.T) {
	...
	eventuallyWithin(t, 5*time.Second, "the revert failed and is owed", func() bool {
		h := s.Health()
	...
	eventuallyWithin(t, 5*time.Second, "the owed resync restored the baseline route and cleared DEGRADED", func() bool {
...
// L3, after TD-11c (verify X1): a failed end of TD-11c's per-transaction claim batch answers DEGRADED,
// which is not stored under the txn_id: after the store recovers, a retry of the same txn_id is APPLIED.
