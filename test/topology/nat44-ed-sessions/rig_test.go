...
// (test/topology/interfaces): the real vrx-agent (owner = slot prefix, VRX_GLOBALS_OWNER=0) and vrx-api on the slot's
...
func newStack(t *testing.T, s slot) *stack {
	...
	apiEnv := append(append([]string{}, base...),
		...
		"VRX_PG_DSN="+pg["VRX_PG_DSN"], "VRX_VALKEY_DB="+s.valkeyDB, "VRX_VALKEY_PREFIX=vrx:"+s.prefix+":nat:"+secret()[:6]+":",
