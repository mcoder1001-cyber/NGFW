...
// CheckPersistent: a record on an untagged interface is ours through the owner's DF-1 claim store.
func (d *RecordDescriptor) CheckPersistent() error { return dfkit.CheckClaims(NameRecord, d.Owner) }

// CheckPersistent: a store on an untagged interface is ours through the owner's DF-1 claim store.
func (d *StoreDescriptor) CheckPersistent() error { return dfkit.CheckClaims(NameStore, d.Owner) }

// CheckPersistent: a mark on an untagged interface is ours through the owner's DF-1 claim store.
