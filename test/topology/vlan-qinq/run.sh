...
# the shared lab lock (the test takes it itself as well, D-094: only for the run). With VRX_QINQ_SHOTS set it also
...
if [[ -n "${VRX_QINQ_SHOTS:-}" ]]; then ( cd "$ROOT/apps/web" && pnpm build >/dev/null ); fi
export VRX_QINQ_AGENT_BIN="$ROOT/apps/agent/bin/vrx-agent" VRX_INTEGRATION=1
cd "$HERE"
