  ...
  it('accepts the implicit default VRF and declared VRFs on interfaces and sub-interfaces', () => {
    expect(
      run('interfaces.vrf-exists', {
        vrfs: { a: { id: 1 } },
        interfaces: {
          loop0: { vrf: 'a', subinterfaces: {} },
          loop1: { subinterfaces: { '10': { vlanId: 10, vrf: 'a' } } },
        },
      }),
    ).toEqual([]);
  });
      ...
      run('interfaces.vrf-exists', {
        interfaces: {
          'TenGigabitEthernet0/0/0': {
            vrf: 'x',
            subinterfaces: { '10': { vlanId: 10, vrf: 'y' } },
          },
        },
      }),
    ).toEqual([
...
describe('interfaces.address-no-overlap', () => {
  ...
  it('reports the identical address twice, on one interface or on two', () => {
    expect(
      run('interfaces.address-no-overlap', {
        interfaces: {
          loop0: { ipv4: ['10.255.0.1/32', '10.255.0.1/32'] },
          loop1: { ipv4: ['10.255.0.1/32'] },
        },
      }),
    ).toEqual([
      {
        pointer: '/interfaces/loop0/ipv4/1',
        message: 'address 10.255.0.1/32 is already assigned at /interfaces/loop0/ipv4/0',
      },
      {
        pointer: '/interfaces/loop1/ipv4/0',
        message: 'address 10.255.0.1/32 is already assigned at /interfaces/loop0/ipv4/0',
      },
    ]);
  });
  it('never crashes on an address that slipped past structural validation', () => {
    ...
    expect(
      interfacesValidators
        .find((v) => v.name === 'interfaces.address-no-overlap')!
        .validate(broken),
    ).toEqual([]);
  });
});
...
describe('interfaces.vlan-unique', () => {
  ...
  it('reports a repeated tag combination on the same parent', () => {
    expect(
      ...
      {
        pointer: `${TEN0}/subinterfaces/200/vlanId`,
        message: 'VLAN dot1q 100 is already used by sub-interface TenGigabitEthernet0/0/0.100',
      },
      {
        pointer: `${TEN0}/subinterfaces/301/vlanId`,
        message: 'VLAN dot1q 300.30 is already used by sub-interface TenGigabitEthernet0/0/0.300',
      },
    ]);
  });
});
    ...
    expect(
      run('interfaces.unnumbered-target-exists', {
        interfaces: {
          ...
          'TenGigabitEthernet0/0/0': {
            unnumbered: 'loop0',
            subinterfaces: { '100': { vlanId: 100, unnumbered: 'loop0.5' } },
          },
        },
      }),
    ).toEqual([]);
    ...
    expect(
      run('interfaces.unnumbered-target-exists', {
        interfaces: {
          ...
          'TenGigabitEthernet0/0/0': {
            subinterfaces: { '100': { vlanId: 100, unnumbered: 'loop9' } },
          },
        },
      }),
    ).toEqual([
      {
        pointer: '/interfaces/loop0/unnumbered',
        message: 'an interface cannot be unnumbered to itself',
      },
      {
        pointer: `${TEN0}/subinterfaces/100/unnumbered`,
...
describe('interfaces.mac-unique', () => {
  it('ignores interfaces without a MAC and accepts distinct MACs', () => {
    expect(
      run('interfaces.mac-unique', {
        interfaces: {
          loop0: {},
          loop1: { mac: '02:00:00:00:00:01' },
          loop2: { mac: '02:00:00:00:00:02' },
        },
      }),
    ).toEqual([]);
  });
  it('reports the same MAC written differently', () => {
    expect(
      run('interfaces.mac-unique', {
        interfaces: { loop1: { mac: '02:00:00:00:00:AA' }, loop2: { mac: '02-00-00-00-00-aa' } },
      }),
    ).toEqual([
      {
        pointer: '/interfaces/loop2/mac',
        message: 'MAC address 02:00:00:00:00:aa is already used by loop1',
      },
    ]);
  });
});
