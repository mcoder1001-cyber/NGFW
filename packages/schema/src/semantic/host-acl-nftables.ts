 ...
 *   acl.host-output-priority  an enabled `output` attachment at priority <= -200: it runs before connection tracking
 *                       (NF_IP_PRI_CONNTRACK), so `ct state established,related accept` never matches there and a drop
 *                       cuts the replies of management sessions (fix round 1, H1; the agent refuses it too)
 *
 ...
 */
/** NF_IP_PRI_CONNTRACK: an output chain must run after it. */
export const HOST_ACL_CONNTRACK_PRIORITY = -200;

export const hostAclNftablesValidators: readonly ValidatorDefinition[] = [
  {
    name: 'acl.host-output-priority',
    domains: ['acl'],
    validate: ({ acl }) => {
      const issues: SemanticIssue[] = [];
      acl.hostAttachments.forEach((a, i) => {
        if (a.enabled && a.chain === 'output' && a.priority <= HOST_ACL_CONNTRACK_PRIORITY) {
          issues.push({
            pointer: jsonPointer('acl', 'hostAttachments', i, 'priority'),
            message: `an output chain at priority ${a.priority} runs before connection tracking (${HOST_ACL_CONNTRACK_PRIORITY}): it would see the replies of established management sessions as new traffic and could cut them; use a priority above ${HOST_ACL_CONNTRACK_PRIORITY}`,
          });
        }
      });
      return issues;
