...
|---|---|---|---|---|---|---|
| `nat64.enable` | `global` | `nat64_plugin_enable_disable` (enable / disable; retval 1 = already, tolerated) | — (empty spec) | `ErrRetrieveUnsupported` (write-only) | — | Sizing (`bib/st buckets`, memory) has no getter → not modelled (VPP defaults). Disable refused (`ErrForeignObjects`) while foreign objects exist. |
