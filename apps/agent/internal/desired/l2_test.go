...
func (s *sink) Add(k scheduler.Key, v proto.Message, _ string) { s.kvs[k] = v }
func (s *sink) Errorf(p, rule, _ string, _ ...any)             { s.errors[p] = rule }
...
func TestMacFilterRoundTrip(t *testing.T) {
	...
	s := &sink{kvs: map[scheduler.Key]proto.Message{}, errors: map[string]string{}}
