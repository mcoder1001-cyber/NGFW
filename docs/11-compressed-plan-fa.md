...
نشانه‌ها: ✅ در طرح — یعنی برنامه‌ریزی‌شده، **نه انجام‌شده**؛ وضعیت انجام در `plan/tasks.yaml` و `docs/status/PROGRESS.md` است و یک قلم فقط وقتی انجام‌شده است که «تعریف انجام‌شدن» بخش ۵ برقرار باشد · 🟡 جزئی (بخش پیکربندی داخل، بخش دیگر خارج) · 🔧 نیاز به کد VPP (ترک جدا) · ⏳ اعتبارسنجی به تعویق (کد هست، صحت روی سخت‌افزار تأیید نشده) · ❌ خارج از طرح
...
| D8.9 Terraform/Ansible/SDK | 🟡 | ۱۶ | Terraform و Python SDK ✅ · Ansible ❌ (Ansible cut: D-085/D-093) — منتظر تأیید مالک محصول (D-125) |
| D9.1 VRRP | ✅ | ۱۳ | |
| D9.2 config sync | ✅ | ۱۳ | |
| D9.3 state sync | 🔧 | — | **V2** |
| D9.4 تست failover | 🟡 | ۱۹ | کارکردی روی VM ✅ · زمان‌سنجی < ۱s ⏳ |
| D9.5 UI خوشه | ✅ | ۱۳ | |
| D10.1 چنداجاره‌ای | ❌ | — | **موکول شد (D-003، ۲۰۲۶-۰۹-۲۳)**؛ پنج ریل در `docs/decisions/vdom.md` هزینهٔ بعدی را ~۱.۵× نگه می‌دارد |
| D10.2 AAA خارجی | ✅ | ۱۶ | |
...
یادآوری (D-125): ✅ یعنی «در طرح»، نه «انجام‌شده». D8.9 از ✅ به 🟡 رفت، چون Ansible حذف شد (D-085/D-093).
...
**دو قاعدهٔ افزوده (D-125، از بازبینی ۲۰۲۶-۰۹-۲۴) — منتظر تأیید مالک محصول:**
- قاعدهٔ ترافیک هر موج: هر موج فقط وقتی بسته می‌شود که ردیف TEST-traffic-<موج> سبز باشد؛ یک سناریوی ترافیکی اسکریپتی روی rig af_packet که همهٔ قابلیت‌های آن موج را پشت سر هم عبور می‌دهد، با شواهد tcpdump، در پنجرهٔ مدیر و با VPP بیکار.
- تعریف انجام‌شدن: قابلیت باید از سند وضعیت مطلوب از طریق API و UI قابل دسترسی باشد — commit از API واقعی با وضعیت applied، notApplied خالی و بدون هشدار agent.unsupported-field، سپس Retrieve()==desired.
