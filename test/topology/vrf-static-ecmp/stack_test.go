...
// F-vrf-static-ecmp copy of test/topology/interfaces/stack_test.go (P08; a test module cannot import another's _test files).
...
// ---- the stack (copy of newStack/startAgent of test/topology/interfaces, work dir "vse") ----------------
...
func newStack(t *testing.T, s slot) *stack {
	t.Helper()
	bin := os.Getenv("VRX_VSE_AGENT_BIN")
	...
	work := filepath.Join(s.runDir, "vse")
	_ = os.RemoveAll(work)
	...
	st.agentEnv = append(append([]string{}, base...),
		...
		"VRX_VPP_TABLE_BASE="+strconv.Itoa(1000*s.num)) // svs tables from the slot range (F-vrf-static-ecmp)
	st.startAgent(t)
	...
	apiEnv := append(append([]string{}, base...),
		...
		"VRX_PG_DSN="+pg["VRX_PG_DSN"], "VRX_VALKEY_DB="+s.valkeyDB, "VRX_VALKEY_PREFIX=vrx:"+s.prefix+":vse:"+secret()[:6]+":",
