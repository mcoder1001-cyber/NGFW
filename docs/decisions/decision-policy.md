...
- **Review request (not a decision):** `REVIEW-<slug>.md` — informational; nothing is parked; the product owner may append `OVERTURNED → …`.
- **Decided by an agent:** one line in `LOG.md` **including the options considered**:
  `| YYYY-MM-DD | D-nnn | decision | options considered | why | reversal cost | tasks affected |`
