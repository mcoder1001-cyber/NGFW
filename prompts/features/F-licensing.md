...
- P06 on `task/P06` (read only): `commit/validation.service.ts` (schema → semantic → agent DryRun pipeline — you add one validation stage),
  `common/problem.ts` (problem+json), `db/schema.ts`, `auth/` (roles)
...
1. **Format** `tools/license/`: licence = canonical JSON `{version, licenseId, customer, issuedAt, notBefore, expiresAt, binding{machineIdHash?|serial?},
   ...
   time, never committed; the product's public key is embedded in the API build via a config file.
2. **API** `apps/api/src/features/licensing/`: `PUT /api/v1/system/license` (admin, upload), `GET /api/v1/state/license` (status: valid / grace /
   ...
   Expiry alarm event on the telemetry bus (F-dashboard-prom-alarms may consume it). OpenAPI; regenerate `packages/api-client`.
...
Files you own: `tools/license/**`, `apps/api/src/features/licensing/**`, `apps/web/src/domains/system/licensing/**`, `apps/web/src/locales/*/licensing.json`,
`docs/user/system/licensing.md`, `test/topology/licensing/**`. Shared files: one-line appends only (`app.module.ts`, the validation-stage list in
P06's validation service, web router/nav) — the manager resolves at merge.
