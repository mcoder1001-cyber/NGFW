...
// nat44EDModels maps each VPP model to its nat44-ed model (installed by New through the TD-23 RegisterExtension registry).
var nat44EDModels sync.Map

func init() {
	RegisterExtension("nat44-ed-sessions", func(v *VPP) { nat44EDModels.Store(v, v.installNat44ED()) })
}
