...
client, owner, wireguard.WithSecrets(resolver))` — it returns the peer descriptor, which is also
...
* The peer index → key map is learnt by Create and Retrieve; an unknown index (after an agent
  restart) is resolved with `wireguard_peers_dump{peer_index}` (the v1 dump — it carries no
  preshared key) plus the interface owner tag.
